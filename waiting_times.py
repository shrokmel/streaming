...
def traj_distance(fils,n=1,t=0.4,sigma=50):
    ...
    for j in range(1250):     
        ...
        # if filament starts diffusive & ends streaming
            ...
            # streaming
            dist = stream_exit - stream_enter[:,:-1]
            dist = (dist**2).sum(axis=0)**.5
    ...
    p, be = np.histogram(running, bins=50)
    bc = (be[:-1]+be[1:])/2.
    ax3.semilogy(bc,p,'o',label=str('thresh='+str(t)))
    ax3.set_xlabel('distance covered streaming')

    #fitfunc = lambda x, a, t: a*np.exp(-x/t)
    #popt, pcov = curve_fit(fitfunc, bc[10:], p[10:])
    #ax3.semilogy(bc, fitfunc(bc,*popt),c='k')
    #print(*popt)
    ...
    #popt, pcov = curve_fit(fitfunc, bc[10:], p[10:])
    #ax4.semilogy(bc, fitfunc(bc, *popt),c='k')
    #print(*popt)
...
def traj_time(fils,n=1,t=0.1,sigma=30):
    ...
    #fitfunc = lambda x, a, t: a*np.exp(-x/t)
    #popt, pcov = curve_fit(fitfunc, bc[10:], p[10:], p0=[1441., 160. ])
    #ax5.semilogy(bc, fitfunc(bc, *popt),c='k')
    #print(*popt)
    ...
    #fitfunc = lambda x, a, t: a*np.exp(-x/t)
    #popt, pcov = curve_fit(fitfunc, bc[10:], p[10:], p0=[1400, 1000])
    #ax6.semilogy(bc, fitfunc(bc, *popt),c='k')
    #print(*popt)
...
def traj_time_vel_corr(fils,n=1,t=0.5,sigma=50, pai=2.0):
    data = fils.xi.transpose(2,1,0) 		# Adopt Guglielmo's convention here to avoid confusion

    # data = [particle ID, (x,y), time] 
    step = data[:,:,n:] - data[:,:,:-n]		# step size dt = n

    vcorr = (step[:,:,1:]*step[:,:,:-1]).sum(axis=1)
    ...
    for j in range(1250):     
        mask = (plt.Normalize()(gaussian_filter1d(vcorr[j],sigma=sigma))>threshold).astype(float)[1:]
        true_to_false = np.diff(mask)
    ...
    p, be = np.histogram(running, bins=100)
    bc = (be[:-1]+be[1:])/2.
    ax5.semilogy(bc,p,'o',label=str('pa='+str(pai)))
    ax5.set_xlabel('time spent streaming')
    ...
    ax6.semilogy(bc,p,'o',label=str('pa='+str(pai)))
    ax6.set_xlabel('time spent in bundle')
...
def traj_distance_vel_corr(fils,n=1,t=0.4,sigma=50):
    data = fils.xi.transpose(2,1,0) 		# Adopt Guglielmo's convention here to avoid confusion
    ...
    for j in range(1250):     
        mask = (plt.Normalize()(gaussian_filter1d(p[j],sigma=sigma))>threshold).astype(float)[1:]
        true_to_false = np.diff(mask)

        # DISTANCE covered
        d = data[j,:,1:-3]		# prune to accomodate true_to_false dimensions
        stream_enter = d[:,true_to_false== 1]
        stream_exit  = d[:,true_to_false==-1]
...
#ax3.set_xlim([0,200])
#ax3.set_ylim([0.8,5000])

#ax4.set_xlim([0,200])
#ax4.set_ylim([0.8,5000])

ax5.set_xlim([0,8000])
ax5.set_ylim([0.8,5000])

ax6.set_xlim([0,8000 ])
ax6.set_ylim([0.8,5000])
...
def main():
    ...
    density=[0.4];	pa=[0.4,0.6,0.8,1.0];	pp=[0.0];
